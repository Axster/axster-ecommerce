...
import { createQueryString } from "../../utils/crateQueryString";
...
export const getMenProducts = async (filters?: Filters) => {
  const query = filters ? `?${createQueryString(filters)}` : "";
...
export const getWomenProducts = async (filters?: Filters) => {
  const query = filters ? `?${createQueryString(filters)}` : "";
  const responses = await Promise.all([
...
export const getProductsByCategory = async (category: CategoryKey, filters?: Filters) => {
  const query = filters ? `?${createQueryString(filters)}` : "";
  const response = await api.get(`${categories[category].url}${query}`);
